...
const generateDrawingDoneHandler = (expectedPlayer, drawingDoneCallback) => (event) => {
  const drawingDoneMessage = parseServerBuffer(event.data);
  if (drawingDoneMessage.player === expectedPlayer) {
    switch (drawingDoneMessage.header) {
      ...
      case serverHeaders.penUp:
        ...
        break;
      case serverHeaders.drawingDone:
        drawingDoneCallback(drawingDoneMessage.imageDataBuffer);
        break;
      default:
    }
  }
...
const startRound = (code, webSocket, round, playerWhoScribbles, myPlayerNumber) => {
    ...
    const gotNextRoundStarting = (nextRoundEvent) => {
      ...
      if (nextRoundHeader === serverHeaders.gameStarting) {
        ...
        startRound(
          code,
          webSocket,
          nextRoundMessage.round,
          nextRoundMessage.whoScribbles,
          myPlayerNumber,
        );
      }
    };
  ...
  if (iScribble) {
    ...
    els.submitDrawingButton.onclick = () => {
      ...
      // a data URL
      const gotExpensionDoneResponse = generateDrawingDoneHandler(
        playerWhoMakesExpension,
        (imageDataBuffer) => {
          webSocket.removeEventListener('message', gotExpensionDoneResponse);
          streamingBoard.drawImageDataBuffer(imageDataBuffer);
    ...
    setScreen('waitingForDrawing');
    const gotScribbleDoneResponse = generateDrawingDoneHandler(
      playerWhoScribbles,
      (imageDataBuffer) => {
        webSocket.removeEventListener('message', gotScribbleDoneResponse);
        drawingBoard.drawImageDataBuffer(imageDataBuffer);
        finalScribbleURL = drawingBoard.toDataURL();
    ...
    const gotGameCode = (gameCodeEvent) => {
      ...
      if (header === serverHeaders.errorMsg) {
        ...
        const otherPlayerJoined = (gameStartEvent) => {
          ...
          if (gameStartMessage.header === serverHeaders.gameStarting) {
      ...
      const gotJoinResponse = (joinResponseEvent) => {
        ...
        if (header === serverHeaders.errorMsg) {
          ...
          startRound(
            code,
            webSocket,
            joinResponseMessage.round,
            joinResponseMessage.whoScribbles,
            1,
          );
        }
      };
